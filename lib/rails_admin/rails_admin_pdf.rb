            ...
            Prawn::Document.new(PDF_OPTIONS) do |pdf|
              ...
              if @object.comissions.where(:status => :pending).count > 0
                ...
                @object.comissions.where(:status => :pending).each do |c|
                  pdf.text(
                    "Id #{c.id}, valor R$#{c.value}, gerada em #{c.created_at.strftime("%d/%m/%y as %H:%M")}",
                    :size => 12, 
:align => :justify, 
:inline_format => true
                  )
...
:inline_format => true
                  ...
                  pdf.text(
                    "Cliente desde #{c.created_at.strftime("%d/%m/%y as %H:%M")}",
                    :size => 12, 
:align => :justify, 
:inline_format => true
                  )
              ...
              pdf.text("Link<link href='https://github.com/Davidsonts/salesRails'>", :size => 10, :inline_format => true, :valign => :bottom, :align => :left)
